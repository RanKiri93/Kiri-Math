...
browsers. Phases 3–4 have not started. The execution handoff for everything that remains is
`kiri-math-handoff.md`.
...
app/
  ...
  courses.ts                   course registry: CourseSummary per course, art attached here
  _site/                       shared shell components; no page.tsx, so never a route
    ...
    CourseEntry.tsx            course-entry transition (client); clicks.ts is the shared plain-left-click test
    ArtSvg.tsx, FadedEquations.tsx
    art/                       pure TS path generators (no React) + tests
  ode/
    ...
    art.ts, art.test.ts        ODE cover, chapter motifs, faded-equation catalog
...
The old module URLs are server redirects. Each module's shell is `<Name>Module.tsx` in its
own folder (`ConstantCoefficientsEulerModule`, `LinearHomogeneousModule`,
`FunctionSequencesSeriesModule`, and the monolithic `phase-plane-module.tsx`). Moving those
folders under `app/ode/` is phase 4c.
...
**Status: built 23 בספטמבר 2026; module «לקריאה ברשימות» links are done. The by-eye check of the native viewer in real browsers is still outstanding.** Differences
...
- Module «לקריאה ברשימות» links are done. Each intro lists the sections that module covers; a wide screen opens the same dialog, and a narrow screen keeps the new-tab link.
...
**Status: built 23 בספטמבר 2026.** Differences from the steps above:

- The transition component is `CourseEntry.tsx`, not `CourseTransition.tsx`. It is a plain `<a>` plus `useRouter()`, and the overlay is portaled to `document.body`.
- The drawing engine is in `app/_site/art/`. The ODE pieces are in `app/ode/art.ts`, and `courses.ts` attaches them to the summary. They are not fields of `CourseDefinition`.
- The cover is the saddle, the slope field with the cubic, and the spring. The figure slot stays empty until the owner's SVG arrives. The card shows four faded formulas; the determinant and the long Wronskian stay on the full catalog only. The dashboard hero has no formulas.
- Chapter 2 is the logistic family and its equilibria. Chapter 3 is the orthogonal parabolas and ellipses, without a direction field. Chapter 6 is the eigenfunctions \(\sin(n\pi x)\); the Sturm–Liouville equation is a faded formula on the cover, not on that header. Empty chapters do not get a second motif beside the empty note.
- Course and chapter art sit at the inline-end edge and hide at ≤820px. On a wide screen the transition title sits in the open area beside the card; on a narrow screen it sits below the drawing. Body text does not cover strokes.
- The cover has 125 strokes, so the draw stagger is 3ms. A 35ms step would run past the 1.5s.
- Checked on vinext and on `next start` (port 3010; 3002 was the dev server), at 1540 / 1180 / 820 / 390. Hex count in `globals.css` stayed 19. Click-to-skip and reduced motion were measured. Esc was not: the embedded browser cannot send a trusted key. Modifier clicks are covered by `clicks.test.ts` only.
